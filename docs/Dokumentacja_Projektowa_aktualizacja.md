...
- Jeżeli spis treści jest utrzymywany ręcznie, warto dopisać sekcje `3.9. Moduł zamówień` oraz `3.10. Moduł profilu i panelu operacyjnego`.
- W aktualnym repozytorium role `bibliotekarz` i `administrator` są obsługiwane tak samo na poziomie uprawnień aplikacji i frontendowych guardów. W DPU najlepiej pokazać wspólnego aktora `Pracownik biblioteki (bibliotekarz / administrator)` albo specjalizację `Administrator` po aktorze `Bibliotekarz`.

## 1.1. Cel projektu

```text
Celem projektu jest zaprojektowanie i zaimplementowanie webowego systemu bibliotecznego, który łączy publicznie dostępny katalog książek z modułami obsługi czytelnika oraz panelem operacyjnym dla pracowników biblioteki. System ma umożliwiać anonimowym użytkownikom przeglądanie zbiorów, zarejestrowanym czytelnikom samodzielne zarządzanie rezerwacjami, wypożyczeniami, karami i powiadomieniami, a pracownikom biblioteki bieżącą obsługę obiegu książek oraz monitorowanie stanu systemu.

Projekt rozwiązuje problem rozproszenia informacji o dostępności książek, kolejce oczekujących, lokalizacji egzemplarzy oraz statusie bieżących zobowiązań czytelnika. Zamiast tradycyjnego, ręcznie prowadzonego obiegu danych użytkownik otrzymuje jeden spójny system, w którym może sprawdzić dostępność pozycji, założyć konto, utworzyć rezerwację, śledzić własne wypożyczenia i przeglądać powiadomienia, a pracownik biblioteki może obsługiwać zwroty, kary, zamówienia i listy użytkowników.
...
3. Dla pracownika biblioteki: zapewnienie operacyjnego panelu pracy obejmującego wypożyczenia, realizację rezerwacji, rozliczanie kar, podgląd użytkowników, zamówienia oraz przegląd danych katalogowych.

Końcowym efektem projektu jest aplikacja internetowa, której backend udostępnia REST API dla modułów katalogowych i obiegowych, a frontend zapewnia publiczny katalog oraz dostęp do funkcji czytelnika i pracownika biblioteki zależnie od roli użytkownika.
```
...
6. Zamówienia i panel operacyjny:
- tworzenie zamówień przez pracownika biblioteki dla książek istniejących w katalogu oraz dla nowych tytułów spoza katalogu,
- obsługa statusów zamówień,
- operacyjny dashboard z licznikami i listami danych systemowych,
...
- frontend aktywnie wykorzystuje te dane głównie w trybie przeglądania i raportowania, bez pełnego graficznego modułu CRUD dla całego katalogu.
...
## 2.1.3. Bibliotekarz (Administrator)

```text
Pracownik biblioteki to użytkownik posiadający rolę bibliotekarza albo administratora. W aktualnym repozytorium obie role są obsługiwane jednakowo na poziomie uprawnień, dlatego funkcjonalnie tworzą jednego aktora systemowego. Taki użytkownik ma dostęp do wszystkich funkcji czytelnika oraz do modułów operacyjnych i administracyjnych.

Pracownik biblioteki może przeglądać listy użytkowników, wypożyczeń, rezerwacji, kar, zamówień, powiadomień i danych katalogowych z poziomu panelu operacyjnego. Może tworzyć wypożyczenia, rejestrować zwroty, oznaczać wypożyczenia jako przeterminowane, wydłużać terminy zwrotu, realizować rezerwacje, rozliczać kary oraz obsługiwać zamówienia książek. Backend udostępnia mu również operacje CRUD dla katalogu i użytkowników, choć aktualny frontend udostępnia głównie widoki list i panel raportowy.

Warto podkreślić, że w aktualnej wersji projektu administrator nie posiada oddzielnego zestawu przypadków użycia względem bibliotekarza. Jeżeli dokumentacja lub diagramy mają rozróżniać te role, najlepiej pokazać administratora jako specjalizację pracownika biblioteki, a nie jako całkowicie odrębnego aktora biznesowego.
```
...
```text
Moduł autoryzacji operacji obejmuje rejestrację, logowanie, wylogowanie, odświeżanie sesji, pobieranie danych bieżącego użytkownika oraz ochronę widoków i endpointów zależnie od roli. Frontend wykorzystuje trzy guardy: `guestGuard`, `authGuard` oraz `staffGuard`, natomiast backend kontroluje dostęp za pomocą klas uprawnień i sprawdzania roli użytkownika.

Nowy użytkownik może zarejestrować wyłącznie konto czytelnika. Role bibliotekarza i administratora nie są nadawane przez publiczny formularz rejestracji. Po zalogowaniu użytkownik otrzymuje token dostępu i token odświeżania oraz odpowiednie ciasteczka HTTP-only, które służą do utrzymania sesji.

W aktualnym repozytorium role `bibliotekarz` i `administrator` są funkcjonalnie traktowane jako wspólny zbiór uprawnień `staff`. Czytelnik ma dostęp tylko do własnych danych, a pracownik biblioteki do danych całego systemu w ramach modułów operacyjnych.
```
...
## 3.10. Moduł profilu i panelu operacyjnego

```text
Aktualne repozytorium zawiera dwa dodatkowe obszary funkcjonalne, które nie powinny być pomijane w opisie przypadków użycia: profil użytkownika oraz panel operacyjny pracownika biblioteki.

Moduł profilu pozwala zalogowanemu użytkownikowi pobrać i zaktualizować własne dane oraz sprawdzić aktualne podsumowanie konta. Dla czytelnika obejmuje ono własne aktywne wypożyczenia, rezerwacje, kary i powiadomienia. Dla pracownika biblioteki podsumowanie odzwierciedla widoczny zakres danych systemowych.

Panel operacyjny pracownika biblioteki agreguje liczniki i listy dla książek, wypożyczeń, zaległości, użytkowników, zamówień, kar i powiadomień. Dodatkowo repozytorium udostępnia listę użytkowników z podsumowaniem aktywności. Te dwa widoki stanowią aktywnie używaną część aplikacji i powinny być wymienione w dokumentacji jako osobne przypadki użycia.
```
...
W aktualnym repozytorium nie ma modułu prowadzenia użytkownika po trasie do regału ani złożonego kreatora obsługi biblioteki. Opis użyteczności powinien więc opierać się na rzeczywiście dostępnych widokach: katalogu, kolejce, wypożyczeniach, karach, powiadomieniach, profilu, zamówieniach i panelu operacyjnym.
```
...
```text
Diagram przypadków użycia dla aktualnej wersji systemu powinien pokazywać trzech aktorów: Gościa, Czytelnika oraz Pracownika biblioteki (bibliotekarz / administrator). Gość korzysta wyłącznie z części publicznej, Czytelnik z modułów samoobsługowych, a Pracownik biblioteki z funkcji operacyjnych i administracyjnych. Warto zaznaczyć, że w aktualnym repozytorium role bibliotekarza i administratora są realizowane tym samym zakresem uprawnień, dlatego w DPU można je pokazać jako jednego aktora albo jako aktora bazowego i jego specjalizację.

Diagram powinien obejmować nie tylko katalog, wypożyczenia, rezerwacje i kary, ale również publiczne przeglądanie systemu przez gościa, obsługę profilu, powiadomienia, zamówienia oraz panel operacyjny. To właśnie te obszary są aktywnie obecne w repozytorium i są widoczne w trasach frontendu oraz w endpointach backendu.
```
...
1. Gość
2. Czytelnik
3. Pracownik biblioteki (bibliotekarz / administrator)
...
12. Zarządzanie katalogiem, egzemplarzami i lokalizacjami

Powiązania aktorów z przypadkami użycia:
- Gość: 1, 2, 3
- Czytelnik: 1, 2, 3, 4, 5, 6, 7, 8, 9
- Pracownik biblioteki: 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12
```

### Wypełnienie placeholderów „Przypadek użycia nr 1-11”
...
Przypadek użycia nr 10: Tworzenie i obsługa zamówień książek przez pracownika biblioteki.
Przypadek użycia nr 11: Korzystanie z panelu operacyjnego, przeglądu użytkowników i danych katalogowych.

Jeżeli diagram ma dokładniej odwzorowywać repozytorium, warto dodać jeszcze osobny dymek „Zarządzanie katalogiem, egzemplarzami i lokalizacjami”, ponieważ backend udostępnia te operacje jako odrębny moduł API.
```
...
- 49 backendowych przypadków testowych w katalogu `backend/library/tests`,
- 55 scenariuszy frontendowych w plikach `*.spec.ts` w katalogu `web/src/app`,
...
- tworzenie książki z autorami i kategoriami,
...
- profil użytkownika,
- listę zamówień,
- dashboard i listę użytkowników,
